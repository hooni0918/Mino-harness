...
| [check_consistency.py](scripts/check_consistency.py) | 결정론적 정합(호명·죽은 링크·axe 명령·agentType 실재·금지 문자열)을 CI에서 기계로 검사 |
